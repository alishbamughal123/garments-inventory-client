...
const SizeChartCard = ({
  ...
  if (loading) {
    return (
      <SurfaceCard className={`p-4 sm:p-6 animate-pulse ${className}`}>
  ...
  // If no size chart exists for this style, display clean empty state
  if (!chartData || !chartData.sizes || chartData.sizes.length === 0) {
    return (
      <SurfaceCard className={`p-6 sm:p-8 text-center space-y-3 sm:space-y-4 border border-dashed border-slate-300 bg-slate-50/50 rounded-2xl ${className}`}>
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center mx-auto border border-amber-200 shadow-2xs">
        ...
        <div className="max-w-md mx-auto space-y-1">
          ...
          <p className="text-[11px] sm:text-xs text-slate-500 font-medium leading-relaxed">
            {isNo
              ? `Det er foreløpig ikke opprettet noe offisielt måleskjema for ${styleName || `Style #${styleNumber}`}.`
              : `There is no size chart added for ${styleName || `Style #${styleNumber}`} yet.`}
          </p>
        </div>
      </SurfaceCard>
    );
  ...
  return (
    <SurfaceCard
      className={`p-3.5 sm:p-6 lg:p-7 space-y-4 sm:space-y-5 border border-slate-200/90 shadow-xs relative transition-all duration-200 ${
        isFullscreen ? "fixed inset-2 sm:inset-4 z-50 overflow-y-auto bg-white shadow-2xl p-4 sm:p-8" : ""
      } ${className}`}
    >
      {/* HEADER SECTION */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-slate-100 pb-3 sm:pb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-slate-500 font-medium">
                {isNo
                  ? "Nøyaktige mål i centimeter (cm) med fargekoder."
                  : "Exact measurements in centimeters (cm) with color badges."}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 self-start sm:self-auto">
          <div className="bg-slate-100 p-0.5 sm:p-1 rounded-xl flex items-center gap-0.5 sm:gap-1 border border-slate-200/70 text-[11px] sm:text-xs font-semibold">
            <button
              type="button"
              onClick={() => setViewMode("matrix")}
              className={`px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg transition-all flex items-center gap-1 cursor-pointer ${
                viewMode === "matrix"
            ...
            >
              ...
              <span>{isNo ? "Enkeltstr." : "Single"}</span>
            </button>
          ...
          {showFullscreenBtn && (
            <button
              ...
              className="p-1.5 sm:p-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-900 shadow-2xs transition cursor-pointer"
      ...
      {viewMode === "matrix" && (
        <div className="space-y-3 sm:space-y-4">
          <div className="overflow-x-auto rounded-xl sm:rounded-2xl border border-slate-200 bg-white shadow-2xs">
            <table className="w-full text-xs text-left border-collapse min-w-[550px] sm:min-w-[700px]">
              <thead>
                <tr className="bg-slate-800 text-white font-bold tracking-wider">
                  <th className="py-2.5 sm:py-3.5 px-3 sm:px-4 sticky left-0 z-20 bg-slate-800 min-w-[160px] sm:min-w-[200px] border-r border-slate-700">
                    <div className="flex items-center gap-1.5 text-slate-200 text-[11px] sm:text-xs">
                      <Ruler className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-400 shrink-0" />
                  ...
                  {chart.sizes.map((sz) => {
                    ...
                    return (
                      <th
                        key={sz.key}
                        className="py-2 sm:py-3 px-2 sm:px-3 text-center transition-all min-w-[60px] sm:min-w-[75px]"
                      >
                        <div className="flex flex-col items-center gap-0.5 sm:gap-1">
                          <span className="font-mono text-xs sm:text-sm font-extrabold text-white">{sz.label}</span>
                          <span
                            className="text-[9px] sm:text-[10px] font-extrabold px-1.5 sm:px-2 py-0.2 rounded-full capitalize shadow-xs border"
                            style={{
                              backgroundColor: bg,
                              color: textColor,
                  ...
                  <th className="py-2.5 sm:py-3.5 px-2 sm:px-3.5 text-center font-bold text-slate-300 min-w-[75px] sm:min-w-[90px] border-l border-slate-700 bg-slate-800 text-[10px] sm:text-xs">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2 sm:gap-2.5">
                          ...
                          <div>
                            ...
                            {row.name !== displayName && (
                              <span className="text-[9px] sm:text-[10px] text-slate-400 block font-normal">{row.name}</span>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      {chart.sizes.map((sz) => {
                        ...
                        return (
                          <td
                            key={sz.key}
                            className="py-2.5 sm:py-3 px-2 sm:px-3 text-center font-mono font-bold text-xs text-slate-700"
                          >
                            {val}
                          </td>
                        );
                      ...
                      {/* Tolerance */}
                      <td className="py-2.5 sm:py-3 px-2 sm:px-3 text-center font-mono text-[10px] sm:text-[11px] font-bold text-slate-500 bg-slate-50/80 border-l border-slate-200/80">
                        <span className="bg-slate-200/80 px-1.5 py-0.5 rounded text-slate-700">
                          {row.tolerance || "—"}
                        </span>
                      </td>
                    </tr>
          ...
          {/* Applicable style footnote */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between text-[11px] sm:text-xs text-slate-500 gap-1.5 pt-0.5">
            ...
            <span className="text-[10px] sm:text-[11px] text-slate-400">
              {isNo ? "Mål er oppgitt i cm." : "All measurements in cm."}
            </span>
          </div>
        </div>
      )}
      ...
      {viewMode === "single" && (
        <div className="space-y-4 sm:space-y-5">
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            <span className="text-[11px] sm:text-xs font-bold text-slate-600 uppercase tracking-wider mr-1">
              {isNo ? "Størrelse:" : "Size:"}
            </span>
            {chart.sizes.map((sz) => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1.5 sm:px-3.5 sm:py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 sm:gap-2 border cursor-pointer ${
                    isSelected
                      ? "shadow-xs scale-105"
                ...
                >
                  <span>{sz.label}</span>
                  <span
                    className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full border border-black/20"
          ...
          {(() => {
            ...
            return (
              <div className="space-y-3 sm:space-y-4">
                <div
                  className="p-3 sm:p-4 rounded-xl sm:rounded-2xl border flex items-center justify-between gap-2"
                  style={{
                ...
                >
                  <div className="flex items-center gap-2.5 sm:gap-3">
                    <div
                      className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center font-black font-mono text-xs sm:text-sm shadow-2xs border shrink-0"
                      style={{
                    ...
                    <div>
                      ...
                      <p className="text-[10px] sm:text-xs text-slate-600 font-medium">
                        {isNo ? "Fargemerke:" : "Colour Tag:"}{" "}
                        <strong className="capitalize">{activeSizeObj.colorBadge}</strong>
                      </p>
                    </div>
                  </div>

                  <span className="px-2.5 py-1 bg-white/80 border border-slate-200/80 rounded-xl text-[10px] sm:text-xs font-bold font-mono text-slate-800 shadow-2xs shrink-0">
                    {chart.unit || "cm"}
                  </span>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-right pl-2">
                          <span className="text-sm sm:text-base font-black font-mono text-blue-600">{val}</span>
                          <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 ml-1">{chart.unit || "cm"}</span>
                        </div>
                      </div>
                    );
                  })}
      ...
      {viewMode === "guide" && (
        <div className="space-y-3 sm:space-y-4 text-xs text-slate-700 leading-relaxed bg-slate-50 p-3.5 sm:p-5 rounded-2xl border border-slate-200">
          <div className="flex items-center gap-1.5 font-bold text-slate-900 text-xs sm:text-sm mb-1 sm:mb-2">
            <Sparkles className="w-4 h-4 text-blue-600 shrink-0" />
            <span>
              {isNo ? "Hvordan ta mål av plagget (Måleveiledning):" : "How to Measure Your Garment:"}
            </span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 sm:gap-3.5">
            <div className="p-2.5 sm:p-3 bg-white rounded-xl border border-slate-200/70">
              <p className="font-bold text-slate-900 text-[11px] sm:text-xs mb-0.5">
            ...
            <div className="p-2.5 sm:p-3 bg-white rounded-xl border border-slate-200/70">
              <p className="font-bold text-slate-900 text-[11px] sm:text-xs mb-0.5">
                {isNo ? "C & D - Halsåpning (Neck Openings):" : "C & D - Neck & V-Neck:"}
              </p>
              <p className="text-slate-500 text-[10px] sm:text-xs">
                {isNo
            ...
            <div className="p-2.5 sm:p-3 bg-white rounded-xl border border-slate-200/70">
              <p className="font-bold text-slate-900 text-[11px] sm:text-xs mb-0.5">
                {isNo ? "E - Rygglengde (Back length):" : "E - Back length:"}
              </p>
              <p className="text-slate-500 text-[10px] sm:text-xs">
                {isNo
            ...
            <div className="p-2.5 sm:p-3 bg-white rounded-xl border border-slate-200/70">
              <p className="font-bold text-slate-900 text-[11px] sm:text-xs mb-0.5">
                {isNo ? "F & H - Erme (Sleeve Specs):" : "F & H - Sleeve Specs:"}
              </p>
              <p className="text-slate-500 text-[10px] sm:text-xs">
                {isNo
            ...
            <div className="p-2.5 sm:p-3 bg-white rounded-xl border border-slate-200/70">
              <p className="font-bold text-slate-900 text-[11px] sm:text-xs mb-0.5">
                {isNo ? "I - Avstand brystlomme (Pocket):" : "I - Chest Pocket Distance:"}
              </p>
              <p className="text-slate-500 text-[10px] sm:text-xs">
                {isNo
