...
const EditLeadPage = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        rank: formData.rank !== "" && formData.rank !== null ? parseInt(formData.rank, 10) : null,
        revenueMnok: formData.revenueMnok !== "" && formData.revenueMnok !== null ? parseFloat(formData.revenueMnok) : null,
        expectedDealValue: formData.expectedDealValue !== "" && formData.expectedDealValue !== null ? parseFloat(formData.expectedDealValue) : 0,
      };

      await updateLead(id, payload);
      toast.success(isNo ? "Lead oppdatert!" : "Lead updated successfully!");
      navigate(appRoutes.crmLeadDetails(id));
    } catch (error) {
      toast.error(
        error?.response?.data?.message || (isNo ? "Oppdatering feilet" : "Update failed")
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6">
      <PageHeader
        title={isNo ? "Rediger B2B Lead" : "Edit B2B Lead"}
        description={
          isNo
            ? `Oppdater bedriftsdata, kontaktinformasjon og kleskrav for ${formData.companyName || formData.fullName}.`
            : `Update business profile, contacts, and garment requirements for ${formData.companyName || formData.fullName}.`
        }
      />

      <form onSubmit={submit} className="space-y-6">
        {/* Section 1: Company Profile & Market Segment */}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6 space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3 pt-2">
            <div>
              ...
              <select
                name="source"
                value={formData.source || "TRADE_SHOW"}
                onChange={handleChange}
                className={inputClass}
              >
            ...
            <div>
              ...
              <input
                ...
                value={formData.expectedDealValue ?? 0}
                onChange={handleChange}
                className={inputClass}
                placeholder="250000"
              />
        ...
        <div className="flex justify-end gap-3 pt-2">
          <Button
            type="button"
            variant="secondary"
            onClick={() => navigate(appRoutes.crmLeadDetails(id))}
          >
            {isNo ? "Avbryt" : "Cancel"}
          </Button>
          ...
          <Button type="submit" disabled={saving}>
            {saving
              ? (isNo ? "Lagrer..." : "Saving...")
              : (isNo ? "Oppdater Lead" : "Update Lead")}
          </Button>
        </div>
      </form>
    </div>
