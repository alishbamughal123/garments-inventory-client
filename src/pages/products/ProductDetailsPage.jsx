  ...
  return (
    <MainLayout>
      <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    ...
                    {t("Available Garment Colours")}
                  </h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {t("Click a colour option to view garment pictures and available size variants.")}
                  </p>
                </div>
                <span className="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 text-xs font-bold">
                  {availableColors.length} {isNo ? (availableColors.length > 1 ? "farger" : "farge") : `Colour${availableColors.length > 1 ? "s" : ""}`}
                </span>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[11px] text-slate-500 flex items-center justify-between border-t border-slate-100 pt-2 font-medium">
                        <span>{grp.variants.length} {isNo ? "størrelser" : "Sizes"}</span>
                        <span className="font-semibold text-slate-700 font-mono">
                          {grp.totalStock} {isNo ? "på lager" : "in stock"}
                        </span>
                      </div>
                    </button>
                  );
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    ...
                    {isNo ? "Tilgjengelige størrelser i" : "Available Sizes in"}{" "}
                    <span className="text-blue-600">{activeColorKey}</span>
                  </h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {t("Select a size to view its exact SKU, Code128 Barcode, and stock level.")}
                  </p>
                </div>
              </div>
                ...
                {activeColorGroup.variants.map((v) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {v.stockQuantity || 0} {isNo ? "stk" : "pcs"}
                      </span>
                    </button>
                  );
                })}
                ...
                <div className="mt-4 p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 rounded-md bg-blue-600 text-white text-xs font-bold font-mono">
                        {isNo ? "Str:" : "Size:"} {activeVariant.size}
                      </span>
                    ...
                    <p className="text-xs text-slate-600 flex items-center gap-2">
                      ...
                      {isNo ? "Hovedstrekkode:" : "Primary Barcode:"}{" "}
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    >
                      ...
                      <span>{t("Print Single Sticker")}</span>
                    </button>
                  </div>
                </div>
              )}
        ...
        {/* 📏 INTERACTIVE RESPONSIVE SIZE CHART & MEASUREMENTS SPEC */}
        <SizeChartCard
          styleNumber={baseStyleNo}
          styleName={product?.styleName || activeVariant?.styleName}
          currentSize={activeVariant?.size}
          defaultView="single"
          embeddedSizeChart={activeVariant?.sizeChart || product?.sizeChart}
        />
        ...
        <SurfaceCard className="p-5 sm:p-8 space-y-6">
          <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
            ...
            <h3 className="text-base font-bold text-slate-900">{t("Full Article Specifications")}</h3>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            <Info label="Stock Quantity" value={`${activeVariant.stockQuantity || 0} ${isNo ? "stk" : "units"}`} />
            <Info label="Min Stock Alert" value={`${activeVariant.minStockAlert || 5} ${isNo ? "stk" : "units"}`} />
          </div>

          {/* Price History if available */}
          {priceHistory.length > 0 && (
            <div className="pt-6 border-t border-slate-100">
              <h4 className="text-sm font-bold text-slate-900 mb-3">{t("Price Revision History")}</h4>
              <div className="overflow-x-auto rounded-xl border border-slate-200">
                <table className="w-full text-xs text-left">
                  <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
                    <tr>
                      <th className="p-3">{t("date")}</th>
                      <th className="p-3">{t("Old Price")}</th>
                      <th className="p-3">{t("New Price")}</th>
                      <th className="p-3">{t("Changed By")}</th>
                    </tr>
                  </thead>
      ...
      <BarcodePrintModal
        ...
        title={
          isNo
            ? `Skriv ut strekkoder - Stil #${baseStyleNo} (${allVariants.length} str./farger)`
            : `Print Barcodes - Style #${baseStyleNo} (${allVariants.length} Sizes/Colors)`
        }
      />
    </MainLayout>
  );
...
const Info = ({ label, value }) => {
  ...
  return (
    <div className="space-y-1">
      <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">{t(label)}</p>
